...
<body>
    ...
    <section class="menu">
        <div class="container">
            <div class="menu-nav2">
                <ul class="clearfix text-center">
                    <?php switch($flag): case "0": ?><li class="active"><a target="" href="<?php echo U('Menu/show?flag=0');?>">全部菜品</a></li>
                            <?php if(is_array($property)): foreach($property as $key=>$pro): ?><li><a target="" href="<?php echo U('Menu/show?flag=1&id='.$pro['id']);?>"><?php echo ($pro["name"]); ?></a></li><?php endforeach; endif; ?>
                            <?php if(is_array($category)): foreach($category as $key=>$cat): ?><li><a target="" href="<?php echo U('Menu/show?flag=2&id='.$cat['id']);?>"><?php echo ($cat["title"]); ?></a></li><?php endforeach; endif; break;?>
                        <?php case "1": ?><li><a target="" href="<?php echo U('Menu/show?flag=0');?>">全部菜品</a></li>
                            <?php if(is_array($property)): foreach($property as $key=>$pro): ?><li <?php if(($id) == $pro['id']): ?>class="active"<?php endif; ?>><a target="" href="<?php echo U('Menu/show?flag=1&id='.$pro['id']);?>"><?php echo ($pro["name"]); ?></a></li><?php endforeach; endif; ?>
                            <?php if(is_array($category)): foreach($category as $key=>$cat): ?><li><a target="" href="<?php echo U('Menu/show?flag=2&id='.$cat['id']);?>"><?php echo ($cat["title"]); ?></a></li><?php endforeach; endif; break;?>
                        <?php case "2": ?><li><a target="" href="<?php echo U('Menu/show?flag=0');?>">全部菜品</a></li>
                            <?php if(is_array($property)): foreach($property as $key=>$pro): ?><li><a target="" href="<?php echo U('Menu/show?flag=1&id='.$pro['id']);?>"><?php echo ($pro["name"]); ?></a></li><?php endforeach; endif; ?>
                            <?php if(is_array($category)): foreach($category as $key=>$cat): ?><li <?php if(($id) == $cat['id']): ?>class="active"<?php endif; ?>><a target="" href="<?php echo U('Menu/show?flag=2&id='.$cat['id']);?>"><?php echo ($cat["title"]); ?></a></li><?php endforeach; endif; break;?>
                        <?php default: ?>
                        <li class="active"><a target="" href="<?php echo U('Menu/show?flag=0');?>">全部菜品</a></li>
                        <?php if(is_array($property)): foreach($property as $key=>$pro): ?><li><a target="" href="<?php echo U('Menu/show?flag=1&id='.$pro['id']);?>"><?php echo ($pro["name"]); ?></a></li><?php endforeach; endif; ?>
                        <?php if(is_array($category)): foreach($category as $key=>$cat): ?><li><a target="" href="<?php echo U('Menu/show?flag=2&id='.$cat['id']);?>"><?php echo ($cat["title"]); ?></a></li><?php endforeach; endif; endswitch;?>
                </ul>
            </div>
            <div class="row">
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response['code'] == 1) {
                        ...
                        str1="document.write(str)";
                        if(str1="对不起，您未登陆无法操作！ 0 /cateringcms/index.php/Home/User/login.html"){
                            swal("失败", "对不起，您未登陆无法操作！", "error");
                        }else{
                            swal("失败", str, "error");
                        }
