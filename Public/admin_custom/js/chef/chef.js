function add_chef() {
    if ($('#signupForm').valid()) {
        formData = $('#signupForm').serialize(); //获取表单全部数据
        $.ajax({
            type: "post",
            url: add_chef_url,
            data: formData,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response['deleteCode'] == 1) {
                        ...
                        $('table#chefmanage').find('tr.chef').eq(liid).remove();
                        setTimeout(function() { location.reload() }, 2000);
                    } else {
